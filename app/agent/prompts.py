...
必须基于下方「知识线索」中的事实作答，引用与用户查询相关的线索；若当前检索到的知识线索不足以回答，须明确说明知识库未覆盖的范围。
输出须在引用事实处标注来源编号，如[1]、[2]（编号对应线索中的标记）。最终回答应该简明扼要，不要长篇大论。
严禁行为：
1. 禁止无依据臆测，注意过滤与问题无关或者明显无效的知识线索。
"""
...
FINAL_CITATION_INSTRUCTION = """请在回答中引用参考资料时，在相关句子末尾标注来源编号，如[1]、[2]（编号对应线索中的标记）。
